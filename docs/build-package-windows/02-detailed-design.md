...
本書は[基本設計書](01-basic-design.md)を受けて、monitor-win-01(Windows Server 2022 Standard、Desktop Experience基準)側のコンポーネント構成・配備手順・アクセス制御・ログ監視・バックアップ/ロールバックを定義します。中央監視host(論理名 monitor-01)側の構成は変更しません。フェーズ1(ホスト単体構築)とフェーズ2(中央監視統合)の区分は[要件定義書](00-requirements.md)のとおりで、フェーズ2は「未実装」2点((1)Dockerホストと対象Windowsホスト間の実L3到達性・windows_exporter側Firewall許可が未確立、(2)`ansible/roles/common_windows`としてコードは追加済みだが実機Windows Serverへの実行実績がゼロ件)が解消するまで`BLOCKED`です。ログ集約経路(FR-05)はWindows側・中央側ともコード追加済みですが、実機Windows Server・実機Lokiへの実行実績はゼロ件で、`WIT-06`は上記(1)と同じ理由で`BLOCKED`のままです(詳細は本書「配備設計」5・7節、[要件定義書](00-requirements.md)参照)。
...
| Grafana Alloy for Windows | Windows側(`ansible/roles/common_windows`)の導入タスク(`tasks/log_collection.yml`)と設定テンプレート(`templates/config.windows.alloy.j2`、既存`compose.yaml`のAlloyと合わせv1.16.1系を基準にする想定)、中央側の`compose.loki-push.yaml.example`(opt-inオーバーレイ)+`deploy/nginx/loki-push.conf.example`(専用ポート・Bearer token認証・送信元IP許可リスト)は、いずれもコードとして追加済み。ただし実機Windows Server・実機Lokiへの実行実績はゼロ件で、バージョン・SHA256・ダウンロードURLは実機決定時に確認する`NOT SET`のまま | 中央Loki(専用の`loki-push-proxy`経由。認証・送信元IP許可は導入済み)、Windows Event Log、IISログファイル | Windows側・中央側ともコードは揃ったが実機実行実績ゼロ件。導入後はGrafanaでLogQL検索できること(WIT-06、現状`BLOCKED`: `WIT-03`と同じ実L3到達性の未確立が理由) |
...
| 中央Loki | 既存filesystem store(変更なし) | Alloy(Linux側は既存稼働、Windows側の導入タスクと中央側の`loki-push-proxy`はコード追加済みだが実機Windows Server・実機Lokiへの実行実績はゼロ件) | 既存のまま`/ready`が200。Windows側ログ検索はWIT-06、`BLOCKED`(実機未検証と、`WIT-03`と同じ実L3到達性の未確立が理由) |
...
5. **ログ収集(コード追加済み・実機未検証)**: Windows側(`ansible/roles/common_windows`)の導入タスク(`tasks/log_collection.yml`)とGrafana Alloy for Windows設定テンプレート(`templates/config.windows.alloy.j2`)、および中央側の`compose.loki-push.yaml.example`(opt-inオーバーレイ)+`deploy/nginx/loki-push.conf.example`(専用ポート・Bearer token認証・送信元IP許可リスト)は、いずれもコードとして追加済みです。ダウンロードURL・SHA256・中央Lokiのpushエンドポイント等の変数がいずれも`NOT SET`の間はWindows側のタスク一式を安全にskipする作りです。中央側のnginxプロキシは実際にnginxプロセスを起動し、401(トークン無し/不一致)・403(許可IP外)・404(未知パス)・204(正しいトークン・許可IPで到達)の各挙動を確認済みですが、実機Windows Server・実機Lokiに対する実行実績はゼロ件です。したがって現時点では実施しません(FR-05、WIT-06、`BLOCKED`: `WIT-03`と同じ実L3到達性の未確立が理由)。
...
7. **中央監視統合(フェーズ2・大部分が未実装、`BLOCKED`)**: `app_node_exporter_targets`へのWindowsホスト1行追加とcentral host側での`site.yml`再適用は「済(自動)」で今すぐ実行できます。Prometheusコンテナは`compose.yaml`上で`monitoring`(`internal: true`)に加えて`host-access`(`internal: true`を付けない通常のbridge network)にも接続されており、nftablesルールの実機検証でも`host-access`側にはMASQUERADEと`DOCKER-FORWARD`chainでのacceptが生成されていることを確認しています。したがって`monitoring`の`internal: true`単体がDockerホスト外への egress を塞いでいるわけではありません。実際にscrapeを成立させるうえで未確立なのは、(a)中央監視host(monitor-01)のDockerホスト自身とWindows Serverが稼働するネットワークセグメントとの実L3到達性(本ラボの各ホストはRFC 5737の例示用アドレス`192.0.2.0/24`を使っており、実ネットワーク上での到達は一度も検証されていません)、(b)windows_exporter側Firewallルールが、`host-access`のMASQUERADEによりWindows Serverから見える送信元がPrometheusコンテナの内部アドレスではなくDockerホスト自身の実IPになる点を踏まえて許可設定されているか、の2点であり、いずれも`NOT SET`です。また、Windowsを既存job名`linux-node`に混ぜること自体が名前と実態の不一致を生む点も残存課題です。blackbox probeの汎用化(FR-04)は`prometheus.yml.j2`の`app_blackbox_probe_targets`変数で解消済みで、IISのhealthエンドポイント等をnode_exporter targetsと同じ「1行追加」の形でprobe対象へ加えられます(WIT-05は対象ホスト未構築に加え、blackbox-exporterから対象Windowsホストへ到達する経路がWIT-03と同じ方向であるため、上記(a)の実L3到達性の未確立にも影響され`BLOCKED`です。同日訂正)。Alloy for Windows導入(FR-05)は、Windows側(`ansible/roles/common_windows`のAlloy導入タスク・設定テンプレート)・中央側(`compose.loki-push.yaml.example`+`deploy/nginx/loki-push.conf.example`)の両方がコードとして追加済みですが、実機Windows Server・実機Lokiへの実行実績はゼロ件で、この経路が実際に機能するには(a)の実L3到達性が別途確立している必要があります。したがって`WIT-06`は「経路が無い」ことによる`BLOCKED`ではなく、`WIT-03`と同じ「実L3到達性が未確立」による`BLOCKED`として扱います。これにより本項目で残る条件は(a)(b)の2点のみとなり、これが解消するまでフェーズ2全体(Ansible role実機実行実績とあわせた「未実装」2点全体としては、本書冒頭および[要件定義書](00-requirements.md)参照)が`BLOCKED`です。
8. **動作確認**: フェーズ1はWIT-01, WIT-02, WIT-04, WIT-08, WIT-09, WIT-10、WST-01〜WST-06、WNW-01〜WNW-09を実施します。フェーズ2はWIT-03, WIT-05, WIT-06, WIT-07, WIT-11を実施しますが、いずれも上記7の実L3到達性の解消(およびAnsible role実機実行実績)までは`BLOCKED`です。WIT-05はコード側の制約(blackbox probe対象の汎用化)は解消済みですが、対象ホスト構築だけでなく実L3到達性の確立も揃って初めて実施可能になるため、`WIT-03`と同じ理由で`BLOCKED`のまま`NOT RUN`です(同日訂正)。判定基準は[試験仕様書・結果票](06-test-specification.md)を正本とします。
...
現時点ではWindows Event Log/IISログは中央Lokiへ送られていません。Windows側(`ansible/roles/common_windows`)のAlloy導入タスク・設定テンプレート、中央側(`compose.loki-push.yaml.example`+`deploy/nginx/loki-push.conf.example`)ともコードとして追加済みですが、実機Windows Server・実機Lokiへの実行実績はゼロ件で、`WIT-03`と同じ実L3到達性の確立も別途必要なため、経路としてはまだ機能していません(FR-05はフェーズ2、`WIT-06`は`WIT-03`と同じ理由で`BLOCKED`)。導入後の設計方針はLinux版を踏襲します。
...
アラート確認時の切り分け順「メトリクス → 直近変更 → ログ → プロセス」はLinux版と共通ですが、Windows側はログ集約(フェーズ2、コードは追加済みですが実機未検証のため`BLOCKED`)がまだ使えないため、フェーズ1時点では次の代替手順で一次切り分けを行います。
