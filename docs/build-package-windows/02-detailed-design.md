...
| Grafana Alloy for Windows | Windows側(`ansible/roles/common_windows`)の導入タスク(`tasks/log_collection.yml`)と設定テンプレート(`templates/config.windows.alloy.j2`、既存`compose.yaml`のAlloyと合わせv1.16.1系を基準にする想定)はコードとして追加済み。ただし実機Windows Serverへの実行実績はゼロ件で、バージョン・SHA256・ダウンロードURLは実機決定時に確認する`NOT SET`のまま | 中央Loki(push API。公開・認証設計は中央側で未着手)、Windows Event Log、IISログファイル | Windows側のコードだけでは実行不可。中央側(Loki push API公開・認証設計)が未着手のため経路全体としては引き続き無い状態。導入後はGrafanaでLogQL検索できること(WIT-06、現状`BLOCKED`) |
...
| 中央Loki | 既存filesystem store(変更なし) | Alloy(Linux側は既存稼働、Windows側はコード追加済みだが実機未導入・中央側のpush API公開/認証設計も未着手) | 既存のまま`/ready`が200。Windows側ログ検索はAlloy for Windows導入(Windows側・中央側の双方)まで未対応(WIT-06、`BLOCKED`) |
...
5. **ログ収集(未実装・将来)**: Windows側(`ansible/roles/common_windows`)の導入タスク(`tasks/log_collection.yml`)とGrafana Alloy for Windows設定テンプレート(`templates/config.windows.alloy.j2`)はコードとして追加済みです。ダウンロードURL・SHA256・中央Lokiのpushエンドポイント等の変数がいずれも`NOT SET`の間はタスク一式を安全にskipする作りで、実機Windows Serverへの実行実績はまだありません。加えて、Windows Event Log/IISログをLokiへpushする経路は、Lokiのpush APIをloopback以外から安全に受け付けるための認証・network設計(中央側)に一切着手していないため、Windows側のコードが揃っただけでは経路として成立しません。したがって現時点では実施しません(FR-05、WIT-06、`BLOCKED`)。
...
現時点ではWindows Event Log/IISログは中央Lokiへ送られていません。Windows側(`ansible/roles/common_windows`)のAlloy導入タスク・設定テンプレートはコードとして追加済みですが、中央側(Lokiのpush API公開・認証設計)には未着手のため、経路全体としては引き続き無い状態です(FR-05はフェーズ2、`BLOCKED`)。導入後の設計方針はLinux版を踏襲します。
