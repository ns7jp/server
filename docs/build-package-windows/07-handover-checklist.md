...
| --- | --- |
| 文書パック | 作成済み |
| フェーズ1: 新規構築 WIT-01 / 冪等性 WIT-02 | `NOT RUN`（対象host未指定） |
| フェーズ1: IIS site health WIT-04 | `NOT RUN` |
| フェーズ1: 対象host/管理端末の network WNW-01〜09（WIT-10） | `NOT RUN` |
...
| フェーズ2: host metrics scrape WIT-03 | `BLOCKED`（`compose.yaml` の `monitoring` networkが `internal: true` のため解消まで解除不可。[試験仕様書・結果票](06-test-specification.md)参照） |
...
- [ ] サービス起動・停止・状態確認コマンド（windows_exporter、IIS(W3SVC)）を引き渡した
- [ ] [運用runbook索引](../runbooks/README.md)を確認した（Windows固有の手順は本パックの[構築手順書](05-build-procedure.md)を正本とする）
- [ ] サービス停止復旧演習（WIT-08、D-1相当）を実施し、RTOを記録した
- [ ] バックアップ日時（毎日03:30 Asia/Tokyo）、保持世代（14日）、復元手順（WIT-09）を確認した
- [ ] [変更・ロールバック計画兼記録票](08-change-rollback-plan.md)（一般ルールは[`docs/change-management.md`](../change-management.md)を正本とする）の流れを説明した

## 運用クイックリファレンス

詳細は各文書を正本とし、引き渡し時は実ホストで一度ずつ実行して出力を運用ログへ残します。Linux版のbashコマンドに相当する部分はPowerShellコマンドへ置き換えています。

| 目的 | コマンド / 正本 |
| --- | --- |
| サービス状態一括確認 | `Get-Service windows_exporter, W3SVC \| Format-Table -AutoSize` |
| 停止中サービス抽出 | `Get-Service \| Where-Object Status -ne 'Running'` |
| IIS site状態 | `Get-Website \| Select-Object Name, State, PhysicalPath`（WebAdministrationモジュール） |
...
| 直近エラーログ（Application/IIS） | `Get-WinEvent -LogName Application -MaxEvents 20 \| Where-Object LevelDisplayName -eq 'Error'` |
| バックアップ実行状況 | `Get-WBSummary`（Windows Server Backup） |
| バックアップ直近ジョブ確認 | `Get-WBJob -Previous 1` |
...
| --- | --- | --- |
| IISサイト停止 / health endpoint失敗 | 運用runbook索引に沿って一次切り分け | `NOT SET` |
| windows_exporterサービス停止（フェーズ2有効化後はalert相当） | サービス再起動、直近変更確認 | `NOT SET` |
| RDP一時許可の消し忘れ、Firewall許可事故 | 該当ルールを直ちに削除し記録 | `NOT SET` |
| 証明書秘密鍵・ローカルAdministratorパスワード等の漏えい | 外部公開を止め、秘密値を再発行 | `NOT SET` |
| host障害（VM / ハイパーバイザー障害） | スナップショット復元判断、RPOの確認 | `NOT SET` |
| 復旧見込みがRTO超過 | 状況、影響、次回報告時刻を共有 | `NOT SET` |

## セキュリティ

- [ ] 秘密値（WinRM証明書の秘密鍵、ローカルAdministratorパスワード等）そのものではなく、安全な受け渡し・再発行方法を共有した
- [ ] 不要な一時アカウント、テストデータ、Firewall許可（特にRDPの一時許可(WST-02)）を削除した
- [ ] WinRM、Firewall、公開port(WST-01, WST-04)を確認し、WinRM許可元が管理元CIDR限定で管理されていることを採録した
- [ ] windows_exporterサービスの実行アカウント(WST-03、現状LocalSystem)を記録し、最小権限化が残存課題であることを明記した
...
| 頻度 | 作業 | 記録先 |
| --- | --- | --- |
| 日次 | windows_exporter / IISサービス状態、Firewall許可ルール、直近エラーログ確認 | 運用ログ |
| 週次 | disk使用量増加、未処理alert（フェーズ2有効化後）、Windows Update適用状況 | 週次レビュー |
| 月次 | サービス停止復旧演習（WIT-08）、通知試験（フェーズ2有効化後） | drill記録 |
| 四半期 | バックアップ復元試験（WIT-09）、アクセス棚卸し（ローカルAdministrator / ドメイングループ） | 記録 |
