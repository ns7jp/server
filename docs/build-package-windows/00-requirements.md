...
| 未実装 | 本パックは設計のみを示しており、コードが無いもの(Windows 対応 Ansible role、`compose.yaml` の `monitoring` ネットワークの外部到達、Windows Event Log/IIS ログを Loki へ送る経路) |
...
| FR-03 | CPU/memory/disk などのホストメトリクスを windows_exporter 経由で中央 Prometheus が収集できること(フェーズ 2、要ネットワーク拡張) | WIT-03 | `ansible/roles/app/defaults/main.yml`(`app_node_exporter_targets`)、`ansible/roles/app/templates/prometheus.yml.j2`、`compose.yaml`(`monitoring` ネットワークの拡張が必要) |
