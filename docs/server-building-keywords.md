...
> **はじめて用語に触れる場合**は、先に[サーバー基礎用語集（やさしい版）](server-basics-glossary/README.md)を読むと進みやすくなります。
> あちらはリポジトリに依存しない基礎用語 408 語を、たとえ・覚え方・まちがえやすい点つきで説明します。
> この文書は、その言葉が **このリポジトリのどこに現れるか**を結び付けるための対応表です。
...
- 用語の意味をたとえから覚え直す: [サーバー基礎用語集（やさしい版）](server-basics-glossary/README.md)
