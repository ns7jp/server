...
| windows_exporter | 署名付きMSI導入、Windowsサービス(既定`LocalSystem`。最小権限化はgMSA。2026-09-07に`ad-dc02`で実施・検証済み)。`--collectors.enabled`で`ad,dns,cpu,logical_disk,net,os,service,system`を有効化 | Windows Defender Firewall(9182許可、中央Prometheus hostのIPのみ)、host OS、gMSA用のKDS root key | フェーズ1: `curl.exe http://localhost:9182/metrics`がローカルで200。フェーズ2: 中央Prometheus Targets画面で`up{job="linux-node", host="ad-dc01"}=1`(FR-09、AIT-09、`BLOCKED`) |
...
| 中央Prometheus(フェーズ2、`BLOCKED`) | 既存Compose上のPrometheus(コード変更なし)。scrape対象は`ansible/roles/app/defaults/main.yml`の`app_node_exporter_targets`変数で管理 | `app_node_exporter_targets`変数、windows_exporter(9182到達は`host-access`経由で経路上は可能だが、Dockerホスト↔`ad-dc01`間の実接続・Firewall許可が未検証) | フェーズ2: `up{job="linux-node", host="ad-dc01"}=1`(FR-09、AIT-09、`BLOCKED`: Dockerホスト↔`ad-dc01`間の実接続・Firewall許可が未確立のため) |
...
8. **中央監視統合(フェーズ2・大部分が未実装、`BLOCKED`)**: `app_node_exporter_targets`への`ad-dc01`の1行追加と中央host側での`site.yml`再適用は「済(自動)」で今すぐ実行できますが、次の3点が解消するまでフェーズ2全体は`BLOCKED`です。1点目は、`ansible/roles`配下にWindows対応role(`common_windows`等)が無く、AD DS自体の構築・設定変更をAnsibleで自動化できないことです(本書の配備設計1〜7がすべて「済(手動)」であるのはこのためです)。2点目は、Dockerホストと`ad-dc01`の間に実際のL3到達性が確立されておらず、windows_exporterのFirewall許可(現状`NOT SET`)もDockerホストの実IPを前提に未設定であることです。Prometheusコンテナは`monitoring`(`internal: true`)に加えて非internalなbridge network`host-access`にも接続されており、nftablesルール確認済みのとおり`host-access`経由でDockerホスト自身のnetwork interfaceを介したMASQUERADE/NAT付きegressが機能するため、`internal: true`自体は到達の妨げではありません。ただし本ラボの全ホストはRFC 5737の例示用アドレス(`192.0.2.0/24`)を使用しており、Dockerホストと`ad-dc01`のセグメント間の実接続は確立されておらず、windows_exporterのFirewall許可もDockerホストのNAT後送信元IP(コンテナ内部IPではない)に対して設定する必要があるため、いずれも`NOT SET`のままです。3点目は、Windows Event Log/ADディレクトリサービス監査ログを既存Lokiへ送る経路(Grafana Alloy for Windowsの導入、Lokiのpush APIをloopback以外からも安全に受け付けるための認証・network設計)が無いことです。これら3点は[Windows版パック](../build-package-windows/02-detailed-design.md)と共通の制約です。
