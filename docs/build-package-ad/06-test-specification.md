...
| ANW-05 | 待受port | `Get-NetTCPConnection -State Listen`、`Get-ChildItem Cert:\LocalMachine\My` | 53,88,135,389,445,464,3268,5986,9182が設計どおり待受、3389は既定Disableで非待受。636・3269(LDAPS/GC LDAPS)は待受の有無が`LocalMachine\My`のサーバー認証証明書の有無と整合し、待受時はその証明書(Subject/EKU/期限)を特定できることがPASS(WinRM用自己署名証明書があれば待受する) | NOT RUN | — |
...
| ANW-08 | Windows Defender Firewall | `Get-NetFirewallProfile -PolicyStore ActiveStore`, `Get-NetFirewallRule` | 実効`DefaultInboundAction`がBlock、AD DS自動生成ルールグループ(表示名はOS言語依存)のスコープが設計と一致 | NOT RUN | — |
