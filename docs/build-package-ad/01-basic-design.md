...
- **2台目のDC追加とレプリケーション実測**: `Install-ADDSDomainController`で2台目を追加し、`repadmin /replsummary`・`repadmin /showrepl`でレプリケーション状態を確認する。FSMO役割の一部を`Move-ADDirectoryServerOperationMasterRole`で移譲し、単一障害点を減らす設計を検証する。
  ...
  **DC 1台を停止した可用性試験も2026-09-03に実施済み(ラボ範囲)**です([証跡](../evidence/2026-09-03-ad-dc-outage-drill.md))。ドメインレベル3役割を保持する`ad-dc01`を計画停止したところ、DNS・LDAP・Kerberos・GC・新規オブジェクト作成は`ad-dc02`単独で継続し、PDCエミュレータ/RIDマスター固有の操作(GPMC等)のみ縮退しました(RPCタイムアウト27秒。接続先DCを明示指定すれば回避可能)。dc01復帰後、ディレクトリとして完全復旧するまで**18分31秒**を要し、DNSサービスの復旧だけでは複製が自然収束せず`repadmin /syncall`による強制再同期が必要という発見がありました。FSMO役割の奪取(seize)は`NOT RUN`です。
