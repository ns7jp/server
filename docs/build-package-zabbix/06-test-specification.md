...
ZIT-08の「件数一致」は、[変更・ロールバック計画 7節](08-change-rollback-plan.md#7-データのロールバックpg_restoreによる復元手順)の手順で得た復元後の件数と、`scripts/ops/zabbix-backup.sh`がdumpと同時に(別接続で)記録した`.counts`を比較します。両者が一致すれば`PASS`です。件数がわずかに異なる場合でも、直近の意図したHost/Item追加・削除とFrontend上の内容確認で差の理由が完全に説明でき、それを証跡へ記録できるならPASSとして扱って構いません。理由が説明できない差は`FAIL`として扱い、本番への切り替えは行いません。
