...
大きな承認会議や専任の承認者を置く運用ではなく、PR と Issue だけで小さく回せる
最小限の仕組みにしている。
...
[手放して進める運用キット](work-completion/README.md) は、既存のChange IDに目的・必須条件・見直し時点・保留時の再開条件をひもづける補助です。必要な安全・動作・復旧・記録を確認した範囲で作業を閉じ、追加の任意改善は採否待ちへ分けます。

未実施・失敗・証跡不足は完了へ変えません。タイマーは記録・切り分け・連携の見直しを促し、障害対応を打ち切るものではありません。同梱CLIは入力から次行動を表示するだけで、変更や終了を承認せず、既存の試験・復旧・引継ぎを代替しません。
