...
| 実害 | **依存案件への実害なし**。実ダウンロード量は**約2.7GB、ホストC:の空き28.95→26GB**。[構築・試験結果票](2026-09-07-wsus-build-validation.md)は当初「約365MB」と記載していたが、暴走中に取得したコンテンツのうち724MBをクリーンアップが解放し、別途MSRT 85MBが残存している以上365MBでは説明がつかないため、2026-09-08に結果票側を訂正した（根拠は結果票の該当節）。**ただし2.7GBという値自体の精度は未検証**で、記録から確実に言えるのは「最低でも約809MB以上のGB規模」まで |
