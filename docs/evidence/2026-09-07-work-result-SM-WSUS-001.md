...
| 実害 | **依存案件への実害なし**。実ダウンロード量は本報告書が「約2.7GB、ホストC:の空き28.95→26GB」、[構築・試験結果票](2026-09-07-wsus-build-validation.md)が「約365MB、28.95→28.73GB」と食い違っており**未解決**（実機のC:使用量と`WsusContent`配下の実サイズを再測定して確定させること） |
