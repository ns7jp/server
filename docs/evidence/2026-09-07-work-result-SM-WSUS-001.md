...
| 実害 | **依存案件への実害なし**。実ダウンロード量は**最低でも約809MB以上**（クリーンアップが解放した724MB＋残存するMSRT 85MB。暴走前は承認0件のため全量が暴走中の取得）。**正確な値は`NOT SET`**。本報告書が当初記載した「約2.7GB」は導出過程が未記録のため未検証、結果票が当初記載した「約365MB」は上記下限と両立しないため撤回。ホストC:の空き容量の増減（28.95→26GB）は、同時間帯にコンテンツストア全体の`robocopy`とSUSDBバックアップ269.4MBを`C:\Backup`へ書いているため根拠に使えない。詳細は[構築・試験結果票](2026-09-07-wsus-build-validation.md)の該当節 |
