...
| 実害 | **依存案件への実害なし**。実ダウンロード量は本報告書が「約2.7GB、ホストC:の空き28.95→26GB」、[構築・試験結果票](2026-09-07-wsus-build-validation.md)が「約365MB、28.95→28.73GB」と食い違っており**未解決**。中断後に承認の巻き戻しと`CleanupUnneededContentFiles`（724MB解放）を実施済みのため、**いま測り直しても当時の値は復元できない**。当時のタイムスタンプ付きログ等が見つからない限り「不明」のまま残すこと |
