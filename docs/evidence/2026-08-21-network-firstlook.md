...
- ホスト（WSL2）から `127.0.0.1:8080` 経由での `ss` / `dig` / `traceroute` / `tcpdump` の実行は、当初の理由（`internal: true` によるポート公開の無効化）により意味のある結果が得られないと判断し、実施していない。
- 一方、名前解決の確認は `docker compose exec`（コンテナ内部への直接アクセス）経由で後日実施できた（上記「追記」参照）。`docs/evidence-capture-checklist.md` 優先6が本来意図していた「クライアント視点での経路・名前解決の観察」のうち、名前解決の部分は実質的にカバーできたと考えている。経路（traceroute）と実際のパケット（tcpdump）の観察は、依然としてホスト側からの検証ができないため未実施。
