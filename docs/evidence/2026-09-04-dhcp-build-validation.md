...
| DIT-09 | サービス停止復旧 | PASS | `dhcpd`を`pkill -9`で停止（`ss -lunp`でUDP 67の待受消失を確認）→手動で再起動→`client01`から新規DISCOVERを送出しACKを取得。停止時刻`09:07:43`UTC、復旧（新規DORA正常化）確認時刻`09:07:52`UTC、**RTO ≈ 9秒**（手動検知・手動復旧。自動監視によるRTOではない。DIT-10がSKIP-ENVのため自動アラートは前提にできない） |
| DIT-10 | 監視統合 | SKIP-ENV | このラボに`monitor-01`（Prometheus/Grafana監視サーバー）が存在しないため未実施。`ansible/playbooks/dhcp.yml`自体はnode_exporterを含まない設計（[08-change-rollback-plan.md](../build-package-dhcp/05-build-procedure.md)参照、監視統合は別途`site.yml`側の責務） |
| DIT-11 | バックアップ・復元 | PASS | `dhcpd.conf`・`dhcpd.leases`をバックアップ（`09:08:03`UTC）→`dhcpd.leases`を破損データで上書き→`08-change-rollback-plan.md`の手順どおり`systemctl stop`相当→バックアップから復元→`dhcpd -t`で構文確認（exit 0）→復元後の`dhcpd.leases`のmd5sumがバックアップ取得時と完全一致することを確認→再起動→`client01`から新規DORAが正常に払い出されることを確認（`09:08:27`UTC）。**RTO（バックアップ完了〜復元後DORA正常化）≈ 24秒** |
