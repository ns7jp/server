...
> **実ダウンロード量の記載が本票と報告書で食い違っています（未解決・確定不能の可能性が高い）。** 本票は「約365MB、ホスト空き28.95→28.73GB」、[作業結果・引き渡し報告書](2026-09-07-work-result-SM-WSUS-001.md)7-1節は「約2.7GB、ホスト空き28.95→26GB」と記録しています。いずれにせよ見積もり345GBに対してごく一部で停止できた点、依存案件への実害が無かった点は両記録で一致しています。
>
> **いま`wsus-01`のC:使用量や`WsusContent`配下のサイズを測り直しても、この値は復元できません。** 中断後に承認を555→16→2件へ巻き戻し、`Invoke-WsusServerCleanup -CleanupUnneededContentFiles`で724MBを解放しており、その後のWSUS動作でも両値は変化しているためです。**新たに取った値で当時の記録を差し替えると、実測していない数字を実測として記録することになります。** 中断時点のタイムスタンプ付きログ、スナップショット、または当時の別の測定記録が見つからない限り、**「不明」のまま残してください。**
