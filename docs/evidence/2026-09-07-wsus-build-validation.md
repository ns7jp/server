...
> **実ダウンロード量について（2026-09-08 訂正）。** 本票はもともと「約365MB、ホスト空き28.95→28.73GB」と記載していましたが、**この365MBという値は本試験の他の記録と両立しません**。ただし**正確な値も確定していません**。現時点で言えるのは次のとおりです。
>
> **確定していること: 最低でも約809MB以上を取得している。**
>
> - `ApplyRule()`実行直後の状態は`ApprovedUpdateCount=555`／`NotApprovedUpdateCount=0`であり、それ以前は承認0件でした。WSUSは承認された更新のコンテンツしか取得しないため、**本機のコンテンツはすべてこの暴走中に取得されたもの**です。
> - その18:12の`Invoke-WsusServerCleanup -CleanupUnneededContentFiles`が**724MBを解放**しています。取得していないものは解放できません。
> - 加えてMSRT（85MB）は取得完了して残っています（SIT-05）。
> - 合計で**724 + 85 = 約809MB**。さらに累積更新KB5120242（564MB）の取得途中ぶんと、破棄できなかった**BITSジョブ18件**が保持する未完了ダウンロードが上乗せされます。
>
> **確定していないこと: 実際の取得量。** [作業結果・引き渡し報告書](2026-09-07-work-result-SM-WSUS-001.md)7-1節は「約2.7GB」と記録していますが、**導出過程が記録されておらず未検証**のため、本票はこの値を採用しません。**下限の約809MB以上とだけ記録し、正確な値は`NOT SET`のままとします。**
>
> **ホストC:の空き容量の増減は、この件の根拠に使えません（2026-09-08 に一度この論法を書いたが撤回）。** 理由は2つあります。第一に、可変VHDXは既に確保済みのブロックへゲストの書き込みを収められるため、書き込んだバイト数どおりにホスト側のファイルが増えるとは限りません。第二に、同じ時間帯にダウンロードとは無関係なC:への書き込みが複数ありました——18:02の`robocopy D:\WSUS\WSUSContent → C:\Backup\WSUSContent-20260907-1802 /E`（コンテンツストア全体の複製）、18:06のSUSDBバックアップ269.4MB、IIS構成のXML出力です。したがって「28.95→28.73GB」も「28.95→26GB」も、取得量の裏付けにはなりません。
>
> **いま`wsus-01`を測り直しても当時の値は復元できません**（その後の巻き戻し・クリーンアップ・WSUS動作で状態が変化しているため）。確定させるには、実機に残る当時のBITSジョブ18件のバイト数、`%SystemRoot%\WindowsUpdate.log`／`SoftwareDistribution`のログ、またはHyper-Vのチェックポイント差分VHDXサイズのいずれかを参照してください。
...
> **2026-09-08追記**: 下記のSIT-06・SIT-04の2件は、[2026-09-08の切り分け結果票](2026-09-08-wsus-sit04-sit06-root-cause.md)で原因を特定し、手順書を修正しました。本節の記述は2026-09-07時点の認識として保存します。

- **SIT-06が`FAIL`**。`ApplyRule()`が絞り込みに従わない理由は本試験では特定できていません。WSUS APIの承認ルール適用セマンティクスの調査が必要です。<br>→ *2026-09-08判明: `ApplyRule()`は絞り込みを正しく守る。ルールの分類・製品が0件で保存されており、WSUSがこれを「全分類・全製品」と解釈したことが原因。*
- **SIT-04が部分達成**。`TargetingMode=Client`へ変更後も`Servers`グループへの自己登録に至っていません。クライアントの再レポート周期、または`Pilot`への手動追加との競合が原因の可能性がありますが、切り分けは未了です。<br>→ *2026-09-08判明: クライアント側ターゲティングでは、クライアントが登録し直すたびに所属グループが申告値ただ1つへ置き換わる。`Pilot`への手動追加は次回登録で失われる。GPOの`TargetGroup`を`Pilot`へ変更して解決。*
