...
> **実ダウンロード量の記載が本票と報告書で食い違っています（未解決）。** 本票は「約365MB、ホスト空き28.95→28.73GB」、[作業結果・引き渡し報告書](2026-09-07-work-result-SM-WSUS-001.md)7-1節は「約2.7GB、ホスト空き28.95→26GB」と記録しており、どちらが正しいか実機のログを再確認するまで確定できません。いずれにせよ見積もり345GBに対してごく一部で停止できた点、依存案件への実害が無かった点は両記録で一致しています。確定値は`wsus-01`のC:使用量と`WsusContent`配下の実サイズを再測定して差し替えてください。
...
いずれも実機で通さないと発見できないものです。性質は3つに分かれます。#1・#2・#7・#9は**コマンド自体がエラーになる**型（実行すれば即座に気づける）、#3・#4・#6は**コマンドは成功するのに結果が伴わない**型、#5・#8は**手順そのものが欠落している**型です。後の2つは「エラーが出なかった＝成功」と判断すると誤った状態のまま先へ進むため、実害が大きくなります。
