...
| # | 根拠 |
| --- | --- |
| 1 | **その時間帯に実際に走ったことが記録されている唯一の候補。** 2026-09-07の工程別時刻（[作業結果・引き渡し報告書](2026-09-07-work-result-SM-WSUS-001.md)）は下表のとおりで、**承認ルールの作成は全同期の実行中に完全に収まっている**。`ApplyRule()`は同期完了の約50分後 |
...
2026-09-07の実行順を時刻で並べると次のようになります。

```text
6節 同期設定       15:12〜16:10
9節 本番同期       16:10〜17:14  ←──────────┐ 64分・557件を取り込む初回全同期
7節 GPO作成        16:09〜16:12             │
8節 承認ルール作成  16:12〜16:20  ←─────────┘ 同期の内側に完全に収まる
SIT-05/06          18:00〜18:22             （ApplyRule()は同期完了の約50分後）
```

**作業者は手順書の記載順どおりには実行していません。** 6節の終了直後（16:10）に長い本番同期を先に走らせ、その裏で7節・8節を進めています。手順書は8節→9節の順ですが、実機では9節の同期が先に始まり、8節のルール作成がその中で行われました。

想定する機序は、ルールが保持する分類・製品の参照が、全同期によるカテゴリ・分類ツリーの取り込みで解決できなくなり、WSUSが「0件＝絞り込みなし」として読むというものです。**ただしこれは記録の突合から立てた仮説であり、実機で確かめていません。**

### 確定させるための再現手順（未実施）

**2026-09-07の実行順は「同期の実行中にルールを作成」だったため、逐次実行だけを試すと引き金を外す可能性があります。** 手順書どおりの逐次実行（ケースA）と、実機で実際に起きた同時実行（ケースB）の両方を試してください。所要は初回全同期のぶん（1回あたり1時間程度）です。
...
**ケースB — 2026-09-07と同じ同時実行**

1. 同じく同期前の状態へ戻したWSUSで、先に`StartSynchronization()`で**初回の全同期**を開始する
2. 同期の実行中に（`GetSynchronizationStatus()`が`Running`のまま）8節のとおり承認ルールを作成し、分類・製品・グループを読み戻して記録する
3. 同期の完走を待つ
4. `GetInstallApprovalRules()`からルールを取り直し、分類・製品の件数を読み戻す

**判定**

- どちらかで0件になれば本仮説は成立し、そのケースが引き金を示す
- **ケースAで保持され、ケースBで0件になる**なら、原因は同期そのものではなく「カテゴリ・分類ツリーの取り込み中にルールを保存したこと」であり、手順書8節・9節の**実行順を守ること自体が対策**になる
- 両ケースとも件数が保持されれば本仮説は否定され、残る候補は当日の記録に残っていない別の操作となる
...
- **絞り込みを空にした操作は再現できていません。** API経路の候補のうち`Enabled`のみの`Save()`・`ApplyRule()`・クリーンアップは否定しましたが、**同期は否定しきれていません**（下の項を参照）。当初はWSUSコンソール（GUI）の書き戻しを有力候補としていましたが、当日の記録が「PowerShell Directを全作業で使用」「GUIを使わずAPI経由」であることと矛盾するため取り下げ、**8節と9節の間に走った初回全同期を最有力**としています（本文の追記を参照）。9.2節の実行直前検証は、原因が何であれ被害を止める位置に置いてあります。
