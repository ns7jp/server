...
| 44 | WSUS版パック手順書9節に、承認前のダウンロード量見積もりと、暴走時に停止する手順が欠けていた。この欠落により、実機で`ApplyRule()`を実行した際に557件中555件が全件承認され、**345GB(`GetContentDownloadProgress().TotalBytesToDownload`)のダウンロードが開始**した。ホストC:の空きは28.95GBで、放置すれば同居するドメインコントローラー`ad-dc02`を巻き添えにする状態だった。検知から約1分で`Stop-Service WsusService -Force`により停止し、依存案件への実害はなかった（実ダウンロード量は**最低でも約809MB以上**＝クリーンアップが解放した724MB＋残存するMSRT 85MB。正確な値は`NOT SET`） | 承認ルールが実際に何件を承認するかは、WSUS APIの意味論とルールの保存状態に依存し、手順書の構文レビューでは検証できない | 2026-09-07、`wsus-01`実機でSIT-06実施中に`ApprovedUpdateCount=555`という想定外の値と、その後のコンテンツダウンロード見積もりで発覚。**2026-09-08の切り分けで、`ApplyRule()`は絞り込みを正しく守り、真因はルールの分類・製品が0件で保存されていたこと（WSUSはこれを「全分類・全製品」と解釈する）と判明**（[結果票](2026-09-08-wsus-sit04-sit06-root-cause.md)）。当初「絞り込みが効かなかった」と記録していたが誤り | 重大インシデント(データ消失・巻き添えリスク) | [PR](https://github.com/ns7jp/server/pull/174) |
...
| 46 | WSUS版パック手順書9節が、`wsus-01`自身を`AddComputerTarget()`で`Pilot`コンピューターグループへ手動追加するよう指示していた。**クライアント側ターゲティングでは、クライアントが登録し直すたびに所属グループがGPOで申告した`TargetGroup`ただ1つへ置き換わる**ため、この手動追加は次の登録で失われる。GPOが`Servers`を申告し承認ルールが`Pilot`を対象とする設計は自己矛盾しており、実機ではクライアントへ提示される更新が**0件**になった（SIT-05も成立しない） | `AddComputerTarget()`は成功し、直後に読めば複数グループ所属が確認できる。所属が失われるのはクライアントが次に登録し直した後であり、手順を通した直後の確認では正常に見える | 2026-09-08、`wsus-01`実機で手動追加後にクライアントを強制再登録させ、`MemberOf=[Pilot,Servers,All Computers]`が`[Servers,All Computers]`へ戻り提示更新が0件になることを実測して発覚 | 偽PASS | [PR](https://github.com/ns7jp/server/pull/184) |
