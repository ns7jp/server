...
採用ご担当者さまに最初に見ていただきたい 3 本です。いずれも AI の手順案内を受けながら、本人が手元の Hyper-V 上の VM を操作して確認しました。

| 題材 | 記録 | 何を示すか |
| --- | --- | --- |
| AD の冗長化と復旧 | [構築・試験](2026-09-01-ad-build-validation.md)・[System State 復元](2026-09-02-ad-restore-drill.md)・[FSMO 奪取](2026-09-04-ad-fsmo-seize.md) | 必須 31 項目 PASS、非権威復元、2 台目の DC と役割の奪取 |
| Ubuntu の構築と監視 | [初期構築](2026-09-08-lab-base01-initial-build.md)・[数値監視](2026-09-08-lab-base01-monitoring-practice.md) | 手作業での OS 初期設定と設定不備からの復旧、停止・復帰の監視表示 |
| 原因の切り分け | [WSUS の SIT-04 / SIT-06 原因特定](2026-09-08-wsus-sit04-sit06-root-cause.md) | 件数の突き合わせで前日の仮説を否定し、真因を特定 |
...
## 2026-09-17：性能 CI 保存結果の再評価

[集計修正後の再試験](2026-09-17-performance-rerun.md)では並列4のHTTP502を正しくFAILとして検出。
続く[接続再利用の比較](2026-09-17-upstream-keepalive-comparison.md)では、測定値とDNS・認証・復旧試験を対象版に結び付けて確認できます。

[原本・測定条件・再計算・未確認範囲](2026-09-17-performance-ci-analysis.md)を保存。
HTTP 502 の集計漏れを検出し、並列4/8はHTTPを含む失敗率1%の基準でFAILと再判定した。
GitHub Actionsの当時の成功表示は履歴として維持する。AI支援による分析であり、本人の独立再現ではない。

## 2026-09-08：lab-base01のAnsible入門

[ローカルのファイル管理でchanged2→0→1を確認](2026-09-08-lab-base01-ansible-intro-practice.md)した記録を原画像4枚付きで追加。
Ansible core2.16.3で初回作成・再実行・手動変更の修正を確認した。**ホーム内の2タスクの入門演習**であり、
foundation.yml/site.yml・OS設定・リモートホスト構築・全ロールの冪等性はNOT RUN。
提示Playbookと実行ファイルの同一性、本人の独力説明は未確認として区別する。

## 2026-09-08：lab-base01のD-1アプリ自動復旧

[アプリ再起動0→1・HTTP復帰2秒・後続healthy確認](2026-09-08-lab-base01-d1-practice.md)を原画像4枚付きで追加。
本人の手元VMでPID変化とHTTP200/healthyを確認し、2コンテナ・3ネットワークを撤去した。
**2秒はスクリプトの秒単位HTTP復帰計測で、healthy到達時間や本番RTO保証ではない。**
監視通知・全構成・別ホスト復旧・反復試験はNOT RUN。実行コマンド等の画面未採録部分を明記。

## 2026-09-08：lab-base01のLokiバックアップ・別ボリューム復元

[停止中のアーカイブ取得と別名volumeからのログ2件検索](2026-09-08-lab-base01-restore-practice.md)を原画像7枚付きで記録。
元volumeをreadonlyでバックアップし、復元先を/lokiに使う確認用Lokiの3110番から過去ログ2件を取得。
確認用コンテナを撤去し、元/復元先volume・archiveを保管。**同一VM内の復元で、別ホスト災害復旧ではない。**
全データの完全性・RTO/RPO・外部保管はNOT RUN。展開コマンド等の画面未採録部分を区別する。

## 2026-09-08：lab-base01の保存ログ再読込み

[Loki/Grafanaだけの再作成と過去ログ2件の再表示](2026-09-08-lab-base01-persistence-practice.md)を原画像5枚付きで追加。
コンテナがない状態と4ボリューム残存を確認してから2サービスを作り直し、前回と同じログ2件を再表示、
終了後も4ボリュームが残ることを確認した。**同じホストのボリューム再利用であり、バックアップ復元ではない。**
全データの完全性・Prometheus/Alloyの保存データ再読込み・長期保持はNOT RUN。

## 2026-09-08：lab-base01の6サービスログ監視

[Loki・Alloy・Nginxのログ生成とGrafana検索](2026-09-08-lab-base01-loki-practice.md)を原画像7枚付きで追加。
目印loki-firstを含むGET/401ログ2件と、6コンテナ・4ネットワークの撤去を確認した。
メモリ2GiBの部分構成であり、全10サービス・保存ログ再読込み・外部通知・自動復旧はNOT RUN。
Exploreの入力欄が空だった経緯と、未採録の実行SHA・トンネル終了も区別する。

## 2026-09-08：lab-base01の5サービス数値監視

[Prometheus収集・Grafana表示・手動停止と収集復帰](2026-09-08-lab-base01-monitoring-practice.md)を原画像10枚付きで記録。
メモリ2GiBのため5サービスに限定し、up=1、メモリ指標、ダッシュボードの1→0→1、撤去を確認した。
**全10サービス・Lokiログ・外部通知・自動復旧・長期稼働はNOT RUN**。
SSHトンネル終了、クリップボード後片付け、ボリューム再読込みは未確認として区別する。

## 2026-09-08：lab-base01のDocker最小構成演習

[メモリ・LVM調整、Docker導入、app/nginxの起動・認証・停止再開](2026-09-08-lab-base01-compose-practice.md)を記録した。
本人の手元VM、教材SHA `1e5b3335cdf0d8ca7d0726b4411792be9f38c73a`でpytest 167件成功、
2サービスのHTTP確認と手動停止・再開・撤去を原画像17枚に対応付ける。
**監視全体・Ansible・D-1・Windowsブラウザ表示はNOT RUN**。本人による説明の再確認は未実施。
記録の追加基点mainやCI結果を、VMでの試験対象SHAと混同しない。

## 2026-09-07〜08：手元VMのLinux初期構築演習

[lab-base01の作業結果・21項目照合・原画像42枚](2026-09-08-lab-base01-initial-build.md)を追加した。
本人のHyper-V上で固定IP、SSH鍵認証、sudo、UFW、NTP、自動更新、設定不備からの復旧を確認。
判定はPASS 14 / PASS-ADAPTED 4 / PARTIAL 2 / NOT RUN 1と、T-14代替演習のPASS 1件。
5秒タイムアウトの定量条件は未採録、T-14の/var全体枯渇はNOT RUN、T-18はWindows ACL版。
...
| AD dc02の時刻同期元の是正 | ✅ [2026-09-07](2026-09-07-ad-dc02-time-sync-fix.md)：FSMO奪取でdc02自身がPDCエミュレーターになったにもかかわらず、Windows Time Serviceの`Type`が`NT5DS`（存在しない上位DCを探す構成）のまま残っていたことを発見・是正（`Type: NTP` / `AnnounceFlags: 5`）。実効時刻源（Hyper-Vホスト時刻）自体は変わらず、「意図せず動いていた」状態から「明示的に構成された」状態への変更。修正直後の一過性の退行に見えた事象（`Local CMOS Clock`）を、60秒待って再確認することで誤検知と確定した |
| AD windows_exporterの最小権限化 | ✅ [2026-09-07](2026-09-07-ad-windows-exporter-least-privilege.md)：既定`LocalSystem`で動作していたdc02のwindows_exporterを、gMSA（`CORP\svc-winexp$`）+ `Performance Monitor Users`メンバーシップへ移行。DCにローカルSAMが無いため`LocalAccounts`モジュールがBUILTINグループを扱えないこと、PowerShellがネイティブexe呼び出し時に空文字列引数を消すため`sc.exe`の`password=""`指定に`--%`が必要なことを実機で発見。`StartName`・`State`・AD系メトリクス取得を変更後も確認し、パラメータシート・詳細設計書・引き渡しチェックリストの継続課題を解消した |
...
| WSUS サーバー（`wsus-01`）の構築・試験 SUT-01〜05 / SIT-01〜08 / SST-01〜06 | ⚠️ [2026-09-07](2026-09-07-wsus-build-validation.md)：既存ADドメイン`corp.example.test`へメンバーサーバーとして参加させ、WSUSロール（WID）を構築。フェーズ1必須28 ID中 **26 PASS / 1 FAIL（SIT-06）/ 1 期待結果未達（SIT-04）** で、フェーズ1は**未完了**。初回同期64分・557件、冪等性の全突合値一致、SUSDB復元10.2秒で内容一致を実測。実行して初めて**手順書の誤り・欠落9件**を発見（`cs`コレクター廃止、`-UpdateServer`パラメーター不在、製品名`Windows Server 2022`の不在、分類名のロケール依存、ロール自動生成Firewallルール、`TargetingMode`変更手順の欠落、8節と9節の矛盾、承認前の容量見積もり手順の欠落、`sqlcmd`不在）。**`ApplyRule()`が555件を全件承認し345GBのダウンロードを開始した事象**を検知1分で停止（依存案件への実害なし。実ダウンロード量は**最低でも約809MB以上**で正確な値は`NOT SET`。当初の「約365MB」はクリーンアップが解放した724MBと両立しないため撤回、「約2.7GB」は導出過程が未記録のため未採用）。ホストPCが管理端末・NATゲートウェイ・ハイパーバイザーを兼ねており、独立した管理端末・組織DNS・実TLS証明書での検証は**NOT RUN** |
| WSUS サーバー（`wsus-01`）の SIT-06・SIT-04 原因切り分け | ⚠️ [2026-09-08](2026-09-08-wsus-sit04-sit06-root-cause.md)：2026-09-07に積み残した2件の原因を実機で特定。**SIT-06は手順書の記述自体が誤り**で、`ApplyRule()`は分類・製品の絞り込みを正しく守る（3コホート12件で、絞り込みありは対象のみ4/4承認・対象外0/8、絞り込み空は12/12承認を実測）。真因はルールが**分類0件・製品0件**で保存されておりWSUSがこれを「全分類・全製品」と解釈したこと。555件は同期済み557件から拒否済み2件を引いた数と一致（設計どおりなら87件）。**SIT-04はクライアント側ターゲティングの性質**で、クライアントが登録し直すたびに所属グループが申告値ただ1つへ置き換わり、9節の`Pilot`手動追加が失われて提示更新が0件になっていた。GPOの`TargetGroup`を`Pilot`へ変更して提示0件→1件を実測し、`Servers`向け承認が子の`Pilot`へ継承されることも確認。手順書の誤り3件（`Pilot`手動追加の非永続、`Stop-Service BITS -Force`による`WsusService`巻き添え停止、GPO操作のダブルホップ制約）を追加で発見。**絞り込みが空になった操作自体は再現できておらず**（`.Enabled`のみの`Save()`・`ApplyRule()`自身・クリーンアップは否定。**同期は否定しきれておらず、8節と9節の間に走った初回全同期が最有力候補**）、**フェーズ1の通し再試験は`NOT RUN`**で総合判定は`FAIL`のまま |
| WSUS サーバー（`wsus-01`）の実機ネットワーク検証 SNW-01〜09 | ✅ [2026-09-07](2026-09-07-network-host-validation-wsus.md)：手元Hyper-V上のWindows Server 2022評価版VMと、ホストPCを管理端末として **9/9 PASS**。5986/8530/9182の待受と3389の非待受、WIDが名前付きパイプのみでTCPポートを持たないこと、8530が内部CIDRから到達し9182が拒否されることを実測。依存案件側の`ad-dc01`が削除済みのため、DC 2台への冗長経路の同時確認は**NOT RUN** |
| WSUS サーバー（`wsus-01`）の作業結果・引き渡し報告 | ⚠️ [2026-09-07 SM-WSUS-001](2026-09-07-work-result-SM-WSUS-001.md)：計画対実績、試験集計 26 PASS / 1 FAIL / 1 期待結果未達 / 1 BLOCKED、設計差異10項目、障害3件（うち`ApplyRule()`の全件承認は重大）、手順書修正提案9件、残存リスク、受領判定。**引き渡し判定は`NOT READY`**。依存案件`SM-AD-001`側の`ad-dc02`にdefault routeとDNSフォワーダを追加する変更を行っており、AD版パック文書への反映が未了 |
| DHCPサーバー（`dhcp-01`相当）の構築・試験 DUT-01〜05 / DIT-01〜11 / DST-01,02,04,06 / DNW-01〜09（netnsラボ版） | ✅ [2026-09-04](2026-09-04-dhcp-build-validation-netns-lab.md)・[ネットワーク結果票](2026-09-04-network-host-validation-dhcp-netns-lab.md)：AI支援セッションのサンドボックスコンテナ内のnetwork namespaceラボ（[`labs/dhcp-lab/`](../../labs/dhcp-lab/)）で、commit`ebcae209`にて31 ID中27 ID `PASS`（新規構築・冪等性、DORA実測、固定予約、プール枯渇101台、RENEW(unicast)、再起動後のリース永続化、リース解放、停止復旧RTO≈11秒、バックアップ復元RTO≈16秒、rogue DHCP確認を含む）。2026-09-07に同じ種類のラボを別commit`0974872`で再構築し、DIT-10（監視統合）・DNW-03（DNS）の2 IDを実際にPrometheus/node_exporter/dnsmasqを構築して`PASS`へ切り替え（別commitのため上記27 IDとは合算しない。[追補](2026-09-07-dhcp-dit10-dnw03-followup.md)）。残るDST-03（AppArmor）・DST-05（監査ログ）はこのサンドボックスにAppArmor LSM・journald/rsyslogが無い恒久的な環境制約で`SKIP-ENV`。実行して初めて3件の欠陥・仕様ギャップを発見（`common` roleの`hwclock`パッケージ欠落、isc-dhcp-serverの300秒lease-timeクランプ、UFWがdhcpdのraw socket受信を実際には制御していない）。**独立した物理／VPSホストでの受け入れは`NOT RUN`**。同日、別の独立したセッションがコンテナ内Docker bridge構成で並行実施した記録は[`2026-09-04-dhcp-build-validation.md`](2026-09-04-dhcp-build-validation.md)（31 ID中22 ID PASS） |
