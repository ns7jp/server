...
| AD DC 2台目追加とレプリケーション実測 | ✅ [2026-09-03](2026-09-03-ad-second-dc-replication.md)：`ad-dc02` を追加し、NTDS 複製 5 パーティション失敗 0、SYSVOL 初期同期成功、**サイト内レプリケーション遅延 17.8 秒**を実測。前日の System State 復元が dc01 の SYSVOL から `scripts` を失わせていたこと（単一 DC では露見しない欠損）を発見・修復（LAB-16〜19）。FSMO 移譲・dc02 のフェーズ1相当設定は**NOT RUN** |
