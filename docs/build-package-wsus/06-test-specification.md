...
| SIT-04 | GPO適用とwsus-01自己登録 | サーバー側`TargetingMode`が`Client`であることを確認したうえで、`gpupdate /force`後`gpresult /r /scope computer`と`Get-WinEvent -LogName "Microsoft-Windows-GroupPolicy/Operational"`、WSUSコンソールの`Pilot`グループを確認 | `WSUS-Client-Policy`がエラーなく適用され`wsus-01`が`Pilot`グループへ自己登録(`割り当てられていないコンピューター`のままなら`TargetingMode`が`Server`の疑い。`Servers`のままならGPOの`TargetGroup`が旧値か、ターゲティングcookie(既定1時間)が切れていない) | NOT RUN | — |
...
| SIT-06 | 自動承認ルールの動作確認 | 「Critical and Security Updates - Pilot Auto-Approve」を手動実行(`Enabled=false`のままでは`ApplyRule()`が拒否されるため、実行直前のみ有効化し直後に無効へ戻す)。**実行前後で`GetContentDownloadProgress().TotalBytesToDownload`を比較する** | 分類Critical/Security、製品`Microsoft Server operating system-21H2`、対象`Pilot`の更新のみ自動承認。他は手動承認。承認件数が実行直前に数えた想定対象件数以下で、ダウンロード見積もりが許容量に収まる（**実行前にルールの分類・製品が0件でないことを検証する。0件は「絞り込みなし=全件承認」を意味する**） | NOT RUN | — |
...
| SIT-08 | SUSDB・コンテンツストアのバックアップ・リストア | (1)SUSDB(WID名前付きパイプ経由またはWindows Server Backupでシステム状態取得)(2)コンテンツストア全体(3)IISのWSUS管理サイト構成、を取得して復元。**WIDは別名データベースへの復元をスキーマ検証で拒否する**ため、系統A(WID)では別ボリューム/別ホストへの復元は成立せず、`SUSDB`自身への`WITH REPLACE`復元で検証する(別インスタンスへの復元は系統B・外部SQL Serverでのみ可能) | 3点とも復元後に内容一致。復元前後でテーブル数・主要テーブルの行数が一致し、WSUS APIとClientWebServiceが応答する | NOT RUN | — |
...
| ID | 確認対象 | 主コマンド | 期待結果 | 結果 | 証跡位置 |
| --- | --- | --- | --- | --- | --- |
| SST-01 | Firewall既定Blockと許可経路の最小化 | `Get-NetFirewallProfile`、`Get-NetFirewallRule \| Where Enabled`(各ルールの`Get-NetFirewallPortFilter`・`Get-NetFirewallAddressFilter`まで確認する) | 既定Default Inbound Block。許可はWinRM(5986、管理元CIDR)、WSUSコンテンツ(8530、内部ネットワークCIDR)、windows_exporter(9182、中央Prometheus host IP)のみ。**WSUSロールが自動生成する8530/8531の`RemoteAddress=Any`ルールが無効化されていること** | NOT RUN | — |
