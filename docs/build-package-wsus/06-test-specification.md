...
| SIT-04 | GPO適用とwsus-01自己登録 | サーバー側`TargetingMode`が`Client`であることを確認したうえで、`gpupdate /force`後`gpresult /r /scope computer`と`Get-WinEvent -LogName "Microsoft-Windows-GroupPolicy/Operational"`、WSUSコンソールの`Servers`グループを確認 | `WSUS-Client-Policy`がエラーなく適用され`wsus-01`が`Servers`グループへ自己登録(`割り当てられていないコンピューター`のままなら`TargetingMode`が`Server`の疑い) | NOT RUN | — |
...
| SIT-06 | 自動承認ルールの動作確認 | 「Critical and Security Updates - Pilot Auto-Approve」を手動実行(`Enabled=false`のままでは`ApplyRule()`が拒否されるため、実行直前のみ有効化し直後に無効へ戻す)。**実行前後で`GetContentDownloadProgress().TotalBytesToDownload`を比較する** | 分類Critical/Security、製品`Microsoft Server operating system-21H2`、対象`Pilot`の更新のみ自動承認。他は手動承認。承認件数とダウンロード見積もりが許容量に収まる | NOT RUN | — |
