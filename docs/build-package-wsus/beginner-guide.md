...
`Stop-Service WsusService -Force`で停止。実際にダウンロードされたのは**約2.7GB**で、
見積もり345GBのごく一部にとどまり、依存案件への実害はありませんでした。

> この2.7GBという数字自体、最初は文書ごとに食い違っていました（片方は「約365MB」）。
> どちらが正しいかは、**記録どうしを突き合わせて**決着しています。暴走中に取得した
> コンテンツのうち724MBを後のクリーンアップが解放しており、365MBしか取得していなければ
> 724MBは解放できないためです。**「後から測り直して確かめればよい」ではありません。**
> 巻き戻しとクリーンアップを済ませた後の実機を測っても、当時の値は出てきません。
> 経緯は[結果票](../evidence/2026-09-07-wsus-build-validation.md)にあります。
