...
**「コマンドが成功する」ことと「意図した結果になる」ことは別物**です。見つかった9件は
すべてこの型でした。たとえば、
...
いずれも構文チェックやコードレビューでは絶対に見つかりません。**実際に動かして、結果を
読み戻して確認する**ことでしか検出できない種類の誤りです。
...
`Stop-Service WsusService -Force`で停止。実際のダウンロードは約2.7GBで止まり、
依存案件への実害はありませんでした。
