...
2. 「[2. 全体の流れを1つの図で見る](#2-全体の流れを1つの図で見る)」で、12文書がどの順に
つながっているかを図で確認する
3. 「[3. 迷わないための5フェーズ](#3-迷わないための5フェーズ)」で12文書を5グループに
   圧縮して覚える
...
| 5本目 | [Ansible自動化基盤版](../build-package-ansible/README.md) | `SM-ANS-001` | どの案件でも使い回せるOS+コンテナランタイム基盤を`ans-01`に構築 |
| 6本目 | [DHCP版](../build-package-dhcp/README.md) | `SM-DHCP-001` | 検証用LANセグメントへIPを払い出す`dhcp-01`を構築 |
| **7本目（本パック）** | **WSUS版** | **`SM-WSUS-001`** | **既存ADドメインへWSUSサーバー（`wsus-01`）を追加し集中管理を実現** |

本パックが少しややこしいのは、単に「7番目に作られたから読む」パックではない点です。
...
主眼があり、**08（変更・ロールバック計画）と10（立ち上げと受け入れ試験）は図に現れません**。
この文書では08を③作る・変えるに、10を④確かめるに分類しています。10の中身は
「構築済みのホストで未確認の項目を試験する」ことが中心なためです。[初心者向け学習ガイド](../beginner-learning-guide.md)
...
>
> GPOの「クライアント側ターゲティングを有効にする」で対象グループ名を`Pilot`にすると、
> `wsus-01`は起動時にこの名前を頼りにコンソール側の`Pilot`グループへ自動登録されます。
> OUを移動しただけではコンピューターグループへ登録されず、逆も同様です。
>
> 紛らわしいのは、**GPOのリンク先は`Servers`OU、申告するコンピューターグループ名は`Pilot`**
> と、2つの`Servers`/`Pilot`が交差する点です。`Servers`という名前がOUとコンピューター
> グループの両方に存在しますが、GPOが申告するのは後者のうち`Pilot`のほうです。
> `Servers`コンピューターグループは`Pilot`の親として作り、そこへの承認は子の`Pilot`へ
> 継承されます。
>
> **この「対象グループ名は`Pilot`」という形は、2026-09-07の実機検証での失敗を受けた
> 修正版です。** 当初の設計はGPOで`Servers`を申告し、`Pilot`へは手動で追加するもので
> したが、実機では動きませんでした。WSUSサーバー側の`TargetingMode`が既定の`Server`の
> ままだとGPOで配った対象グループ名が無視され（手順書にこの切り替え手順が欠けていた）、
> `Client`へ直した後も`Servers`に留まりました。2026-09-08に原因を切り分けた結果、
> **クライアント側ターゲティングでは、クライアントが登録し直すたびに所属グループが
> 「申告した名前ただ1つ」へ置き換わる**ことが分かりました。手動追加は次の登録で消えます。
> 詳細は[切り分け結果票](../evidence/2026-09-08-wsus-sit04-sit06-root-cause.md)を参照して
> ください。
...
| 自動承認ルールを作れば、あとは自動的に承認が進む | 本パックの自動承認ルールはスケジュール化せず手動実行にとどめる、無人承認を避ける安全側の設計である。**ただし「手動実行だから安全」ではない**: 2026-09-07の実機検証では、手動実行した`ApplyRule()`が分類・製品・グループの絞り込みを一切無視して557件中555件を承認し、345GBのダウンロードが始まった（SIT-06 `FAIL`）。承認前にダウンロード量を見積もる手順が必須（[結果票](../evidence/2026-09-07-wsus-build-validation.md)、[欠陥台帳](../evidence/defects-found.md)#44） |
| 12文書が揃えば構築完了 | 文書が「作成済み」であることと、実ホストで試験が`PASS`して引き渡しが完了したことは別の状態。2026-09-07にフェーズ1を実機で通しで実施し必須28 ID中26 `PASS`まで到達したが、SIT-06が`FAIL`・SIT-04が期待結果未達のため[案件パックREADME](README.md)の引き渡し判定は`NOT READY`のまま |

## 10. 実機で通してみた結果（2026-09-07）

**一言でいうと**: 文書どおりに実機を動かしてみたら、**手順書の誤り・欠落が9件見つかり、
1件は345GBのダウンロードが暴走する事故になりました**。その記録です。

12文書を書き終えた後、手元のHyper-VにWindows Server 2022評価版のVMを立て、
[05 構築手順書](05-build-procedure.md)を上から順に実行しました。依存元のADドメインは
[AD版パック](../build-package-ad/README.md)の`ad-dc02`（`ad-dc01`は2026-09-04のFSMO奪取演習で
削除済みのため、単一DC構成）をそのまま使っています。

| 区分 | 結果 |
| --- | --- |
| フェーズ1必須28 ID | **26 `PASS` / 1 `FAIL`（SIT-06）/ 1 期待結果未達（SIT-04）** |
| 引き渡し判定 | **`NOT READY`**（フェーズ1未完了） |
| 主な実測値 | 初回同期64分・557件、`wsusutil postinstall` 1.3分、SUSDBバックアップ38.4秒/269.4MB、復元10.2秒で内容一致 |
| 見つかった手順書の誤り・欠落 | **9件**（[欠陥台帳](../evidence/defects-found.md)#37〜45）。2026-09-08の追加調査でさらに3件（#46〜48）が見つかり、**合計12件** |

詳細は[構築・試験結果票](../evidence/2026-09-07-wsus-build-validation.md)にあります。

### この演習で一番大事な学び

9件の誤りは、性質の違う3つの型に分かれます。**どれも構文チェックやコードレビューでは
見つからず、実際に動かして初めて現れる**という点だけが共通しています。
...
- 承認ルールに製品`Windows Server 2022`を指定した → コマンドは成功するが、WSUSカタログに
  ...
  ルールが保存される
- 分類を英語名`Critical Updates`で指定した → 日本語環境ではローカライズされた名前でしか
  一致せず、これも対象0件のまま成功する
- GPOでクライアントの所属グループを配った → サーバー側の`TargetingMode`が既定のままだと
  その指定は無視される（切り替え手順が手順書に無かった）

**型3: そもそも手順が書かれていない**（何も起きないので気づきにくい）

- WSUSロールが自動生成する全許可Firewallルール（8530/Any・8531/Any）を無効化する手順が無い
- 承認前にダウンロード量を見積もる手順と、暴走時に止める手順が無い（後述の345GB事故の原因）

**型1は実行すれば必ず止まるので、実は安全なほうです。** 危険なのは型2と型3で、
「エラーが出なかった＝うまくいった」と判断すると、間違った状態のまま先へ進んでしまいます。
**実際に動かして、結果を読み戻して確認する**ことでしか検出できません。

### 345GBのダウンロードが始まった話

一番危なかったのは自動承認ルールです。「分類はCritical/Security、製品はServer 2022、対象は
`Pilot`グループだけ」と絞り込んだつもりで`ApplyRule()`を手動実行したところ、**557件中555件が
承認され、約345GBのダウンロードが始まりました**。

当初これは「`ApplyRule()`が絞り込みを無視した」と記録していましたが、**2026-09-08の追加調査で
その見立てが誤りだと分かりました**（[原因切り分け結果票](../evidence/2026-09-08-wsus-sit04-sit06-root-cause.md)）。
`ApplyRule()`は絞り込みを正しく守ります。真因は、実行時点でルールに**分類0件・製品0件**が
保存されており、WSUSがそれを「絞り込みなし＝全分類・全製品」と解釈したことでした。555件は
同期済み557件から拒否済み2件を引いた数で、**絞り込みが空だった場合の件数と一致**します
（設計どおりの絞り込みなら対象は87件でした）。

つまりここでの教訓は「APIが言うことを聞かない」ではありません。**「保存したつもりの設定が、
実際には保存されていなかった」**です。前節の型2そのもので、保存後に読み戻して件数を数えて
いれば、承認を実行する前に気づけました。

ホストのCドライブの空きは28.95GBしかなく、そのまま放置すればディスクが枯渇し、
**同じホストで動いていたドメインコントローラー`ad-dc02`まで巻き添えにする**ところでした。
`GetContentDownloadProgress()`で見積もり値を確認して異常に気づき、約1分後に
`Stop-Service WsusService -Force`で停止。実際にダウンロードされたのは見積もり345GBの
ごく一部にとどまり、依存案件への実害はありませんでした。

> **この「ごく一部」が具体的に何MBなのかは、いまも確定していません。** ここは
> 記録の扱い方の練習台になるので、少し詳しく書きます。
>
> 当初、実ダウンロード量は文書ごとに食い違っていました（「約365MB」と「約2.7GB」）。
> このうち**365MBのほうは否定できます**。暴走中に取得したコンテンツのうち724MBを後の
> クリーンアップが解放しており、365MBしか取得していなければ724MBは解放できないためです。
> 残存するMSRT 85MBと合わせて、**最低でも約809MB以上**は取得しています。
>
> しかし**2.7GBが正しいとも言えません**。その数字がどう求められたのかが記録に残って
> いないからです。「片方が誤りだと分かった」ことと「もう片方が正しいと分かった」ことは
> 別です。そこで結果票は**下限の約809MB以上だけを記録し、正確な値は`NOT SET`**の
> ままにしています。
>
> **「後から測り直せばよい」も成り立ちません。** 巻き戻しとクリーンアップを済ませた
> 実機を測っても当時の値は出てきませんし、ホストの空き容量の増減も——同じ時間帯に
> バックアップで大量にコピーしているため——根拠になりません。詳細は
> [結果票](../evidence/2026-09-07-wsus-build-validation.md)にあります。

手順書には**承認前にダウンロード量を見積もる手順も、暴走時に止める手順もありませんでした**。
これが9件の中で最も重い欠落です（[欠陥台帳](../evidence/defects-found.md)#44）。

### その後に分かったこと（2026-09-08の追加調査）

積み残していた2件は、実機で原因を特定できました（[原因切り分け結果票](../evidence/2026-09-08-wsus-sit04-sit06-root-cause.md)）。

- **SIT-06**: 上記のとおり、`ApplyRule()`の不具合ではなくルールの絞り込みが空だったこと。
  手順書9.2節に、実行直前に絞り込みが0件ならエラーで止める検証と、`UpdateScope`で
  想定対象件数を先に数える手順を追加しました
- **SIT-04**: クライアント側ターゲティングでは、クライアントが登録し直すたびに所属グループが
  **「申告した`TargetGroup`ただ1つ」へ置き換わります**。GPOが`Servers`を申告しつつ`Pilot`へ
  手動追加する、という設計自体が成立していませんでした。GPOの申告先を`Pilot`へ変更して解決しています

### それでもまだ終わっていないこと

- **絞り込みが空になった操作そのものは再現できていません。** `.Enabled`のみ変更した`Save()`、
  `ApplyRule()`自身、週次クリーンアップは否定されましたが、**同期は否定しきれていません**
  （試したのは差分ゼロの45秒同期で、当日の64分・557件の初回全同期とは条件が違うため）。
  現在は**8節と9節の間に走ったこの初回全同期が最有力**とされています。そのため
  「原因が何であれ承認前に止まる」検証を手順へ入れる、という形で対処しています
- **フェーズ1の通し再試験は未実施です。** 原因が分かって手順を直しただけで、試験をやり直した
  わけではありません。**総合判定は`FAIL`のまま**です。何をどの順でやり直すのか、始める前に
  何を確かめるのかは[立ち上げと受け入れ試験](10-host-bringup-and-acceptance.md)の8節に
  まとめてあります

「動かしたら全部うまくいきました」ではなく、**どこまで確認できて、どこから先が未解決なのかを
分けて書く**のが、この案件パック全体の方針です。
...
- 用語をさらに詳しく: [未経験者向けサーバー構築キーワード集](../server-building-keywords.md#14-wsus更新プログラム集中管理の基礎)（WSUS・WID・SUSDB・承認ルール・ターゲティングモードなど）
