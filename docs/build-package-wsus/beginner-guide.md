...
> この名前を頼りにコンソール側の`Servers`グループへ自動登録される、という想定です。OUを
> 移動しただけではコンピューターグループへ登録されず、逆も同様です。
>
> **ただし2026-09-07の実機検証では、この自動登録が設計どおりに動きませんでした。**
> WSUSサーバー側の`TargetingMode`が既定の`Server`のままだとGPOで配った対象グループ名が
> 無視され（手順書にこの切り替え手順が欠けていた）、`Client`へ直した後も`Servers`ではなく
> `Pilot`に留まったままです。原因の切り分けは未了で、SIT-04は「部分達成」として
> [結果票](../evidence/2026-09-07-wsus-build-validation.md)に記録しています。
