...
| 見つかった手順書の誤り・欠落 | **9件**（[欠陥台帳](../evidence/defects-found.md)#37〜45） |
...
`Pilot`グループだけ」と絞り込んだつもりで`ApplyRule()`を手動実行したところ、**絞り込みが
一切効かず557件中555件が承認され、約345GBのダウンロードが始まりました**。
...
- **SIT-06（`FAIL`）**: `ApplyRule()`が絞り込みに従わない理由は特定できていません
- **SIT-04（期待結果未達）**: `TargetingMode`を`Client`に直した後も、`wsus-01`の自己登録先が
  `Servers`ではなく`Pilot`のままです
