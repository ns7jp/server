...
| GPO(`WSUS-Client-Policy`) | `Servers`OUへリンクし、自動更新の構成・更新サービスの場所・クライアント側ターゲティングを配布 | `gpresult /r`で適用を確認、グループポリシー操作ログにエラーなし。`wsus-01`がWSUSコンソールの`Pilot`グループへ自己登録(SIT-04) |
...
6. **GPO設計**: `WSUS-Client-Policy`を作成し`Servers`OUへリンクする(ドメイン直下・`_Tier0-Admins`OUへは広げない)。「自動更新を構成する」はオプション3(自動ダウンロードを行い、インストールの通知を行う)を選択する。オプション4の自動インストール・自動再起動ではなく通知にとどめ、無人再起動によるサービス影響を避ける安全側の判断とする。「イントラネット Microsoft 更新サービスの場所を指定する」に検出・統計両方で`http://wsus-01.corp.example.test:8530`を設定する。「クライアント側ターゲティングを有効にする」の対象グループ名は`Pilot`とし、WSUSコンソール側のグループ名と一致させる。クライアント側ターゲティングでは、クライアントが登録し直すたびに所属グループが申告値ただ1つへ置き換わるため、段階展開リングである`Pilot`を直接申告させる(`Servers`向けの承認は親子階層により継承される)。検出頻度は既定のまま変更しない(FR-04、SIT-04)。
7. **コンピューターグループ・承認ルール**: `すべてのコンピューター`配下に`Servers`グループを手動作成(ADのOUとは別概念)、その下にサブグループ`Pilot`を作成し、GPOのクライアント側ターゲティングで`wsus-01`自身を`Pilot`へ自己登録させる(API/コンソールからの手動追加は次の登録で失われるため使わない)。自動承認ルール「Critical and Security Updates - Pilot Auto-Approve」(分類: Critical/Security Updates、製品: `Microsoft Server operating system-21H2`、対象: Pilot)を作成するが、無人承認を避けるためスケジュール化はせず手動実行にとどめる。他は手動承認とする(FR-05、NFR-08、SIT-05、SIT-06)。
