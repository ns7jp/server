...
**同期が走っている最中にこの節を実行しないこと。** 全同期は分類・製品のカテゴリツリーそのものを書き込むため、その最中に保存したルールの分類・製品の参照が解決できなくなり、0件として読まれる疑いがある。2026-09-07の実機は、時間短縮のため9節の全同期(16:10〜17:14)を先取りして開始し、**その実行中にこの節(16:12〜16:20)を進めており**、`ApplyRule()`の時点で絞り込みが0件になっていた。因果は未確定だが、9節の同期を先取りしなければ避けられる。

```powershell
# 同期が走っていないことを確認してからルールを作成する
$wsus.GetSubscription().GetSynchronizationStatus()   # NotProcessing であること
```

なお、この節でルールを作ってから9節の全同期を実行する（手順書どおりの順序）場合にも同じことが起きるかどうかは未確定である。どちらの順序であっても、9.2節の実行直前検証で承認そのものは止まる。
