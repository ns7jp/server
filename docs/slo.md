...
**現状: 2026-09-17 の CI 負荷試験 1 回分を分析済み。HTTP 502 の集計漏れを検出しました。**
...
負荷試験スクリプト `scripts/perf/run-perf.sh` は CI で 1 回実行され、保存結果を分析済みです。
...
負荷試験の保存結果を分析した段階であり、集計修正後の再測定と障害演習は残っています。
