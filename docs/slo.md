...
**現状: 実装済み（未実施 / NOT RUN）。この節に書いてあるのは「確かめ方の手順」だけで、
確かめた結果はまだ 1 件もありません。** 数値が入るのは、実際に走らせたときです。

SLO は本来「そのサービスが壊れるところを見たことがある人」が書くものです。
ところがこの文書は、レイテンシの目標（p95 < 500ms）を掲げている一方で、
**レイテンシが劣化する状況を一度も作っていません**。目標を置いただけで、
その目標が守れる値なのかどうかを確かめていない、という状態です。
この節では、その差をどう埋めるかを手順として書き出します。
...
補足として、レイテンシ分布（p95 など）を狙って測った負荷試験の記録は、現時点では
ありません。負荷試験スクリプト `scripts/perf/run-perf.sh` は実装済みですが未実施です。
したがって「p95 < 500ms は実測に基づく値だ」とは、この時点では言えません。
...
この 3 行はいずれも**判定の枠組み**であって、判定の結果ではありません。
現時点でどれに該当するかは分かりません（未実施 / NOT RUN）。
実行には Docker が動く環境が必要で、まだ実行していません。
...
この節を書いた時点では 1. をまだ実行していないため、**99.5% / p95 500ms / 28 日 /
30 日 / 35 日 の各値は一切変更していません。**実測していない以上、
緩めることも厳しくすることも根拠がないからです。
