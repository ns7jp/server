...
設定不備をわざと起こし、クライアント側の拒否表示とサーバー側のログを照合して復旧させています。
記録は[作業結果・21項目照合・原画像42枚](evidence/2026-09-08-lab-base01-initial-build.md)にあります。
...
- Ansible も本リポジトリのコードも使っていません。`site.yml` / `foundation.yml` の適用とは別です
- 監視ラボの構築案件 `SM-LAB-001` の受け入れではありません
- AI が手順案内・画像の読取り・記録の編集を支援しました。AI が VM へ接続して試験を代行した記録ではありません
- **「教材どおり 21/21 PASS」ではありません。** 代替演習の PASS を元の項目へ加算していません
