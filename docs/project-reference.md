...
| [Windows サーバー構築案件パック](build-package-windows/README.md) | 既存監視基盤へ Windows Server を監視対象ホストとして追加する設計・パラメータ・手順一式（[初心者ガイド](build-package-windows/beginner-guide.md)付き）。フェーズ2（中央監視統合）は、Windows対応Ansible role（`ansible/roles/common_windows`）とGrafana Alloy for WindowsのWindows側導入タスク・設定テンプレートをコードとして追加済みだが、実機Windows Serverへの実行実績はゼロ件。Dockerホスト↔対象Windowsホスト間の実L3到達性・Firewall許可の未確立、中央Loki側のpush API公開・認証設計が未着手であることとあわせ3点の解消待ちで`BLOCKED` |
