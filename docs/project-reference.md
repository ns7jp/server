...
[入口へ戻る](../README.md) / [最初の実習](beginner-learning-guide.md) / [検証証跡台帳](evidence/README.md)

機能や発展教材を探すための参照用文書です。初回の起動は上の「最初の実習」に沿って進めます。以下のコマンドの実行場所はすべてリポジトリ直下です。実装の存在と日付付きの実測結果を分け、検証範囲は証跡の環境・日時・対象 commit で確認してください。
...
| クラウド配備 | Terraform で AWS 上に同等構成をコード化（[詳細](aws-architecture.md)。apply 未実施） |
| SLO 運用 | `/healthz` の定期チェックと、しきい値を超えたときのアラート通知（[詳細](slo.md)） |
| 復旧演習 | D-1 はローカル実測RTO（復旧目標時間。壊れてから直るまでの目標）13秒（[2026-08-19](drills/logs/2026-08-19-D-1.md)）とE2E実測RTO 1秒（[2026-08-22](evidence/2026-08-22-full-stack-e2e.md)）。D-2 はランブック・テンプレートのみで未実測 |
| 変更管理 | PR ごとに目的・影響範囲・ロールバック手順を書いて残す運用（[詳細](change-management.md)） |
| 品質確認 | pytest、構成検証、ansible-lint、Molecule 構文検証、任意実行の完全 Molecule、Terraform 検証、Trivy / pip-audit、Dependabot |
| 一気通貫 E2E | disposable Ubuntuへ`site.yml`を2回適用し、runtime/network/通知/障害/restoreを[2026-08-22に全項目PASS](evidence/2026-08-22-full-stack-e2e.md)。raw logと判定表をartifact化 |
...
初心者が最初に開くのは[一本道ラーニングパス](learning-path.md)、[初心者向け学習ガイド](beginner-learning-guide.md)、[案件パック 初心者ガイド](build-package/beginner-guide.md)の3本です。残りは必要になったときに引きます。

| 文書 | 対象 | 目安 | 内容 |
| --- | --- | ---: | --- |
| [一本道ラーニングパス](learning-path.md) | 🟢 初心者 | まず15分 | 必修Level 0〜5と選択式Level 6、各段階の完了条件 |
| [初心者向け学習ガイド](beginner-learning-guide.md) | 🟢 初心者 | 90分 | 最小構成を確認し、動かし、一次切り分けし、説明する |
| [案件パック 初心者ガイド](build-package/beginner-guide.md) | 🟢 初心者 | 20分 | 案件パックとは何か、12文書の役割、読む順とかかる時間の目安 |
| [Linux サーバー構築案件パック](build-package/README.md) | 🟡 中級 | 半日〜 | 要件から設計、パラメータ、構築、試験、作業結果、引き渡しまでの標準成果物一式 |
| [インフラ監視ラボ設計](architecture.md) | 🟡 中級 | 30分 | 構成図、設計判断、監視条件 |
| [設計判断記録](design-decisions.md) | 🟡 中級 | 20分 | 比較案、採用理由、欠点、見直し条件をADR形式で説明 |
| [失敗から学んだ代表事例](lessons-learned.md) | 🟢 初心者 | 15分 | 実行時欠陥3件の切り分け、修正、再発防止 |
| [セキュリティ設計](security.md) | 🟡 中級 | 30分 | 認証、秘密管理、公開範囲、残存リスク |
| [構築・配備手順](deployment.md) | 🟢 初心者 | 60分〜 | Docker Compose と native Linux 配備例 |
| [Ansible 配備手順](deployment-ansible.md) | 🟡 中級 | 半日〜 | Ubuntu host向け一括構築playbook、roles 構成、Vault、Molecule |
| [新規host一気通貫E2E](e2e-validation.md) | 🔴 発展 | 30分 | `site.yml`適用、冪等性（べきとうせい。何度実行しても同じ状態に落ち着く性質）、network、D-1、backup restoreの自動検証と証跡境界 |
| [バックアップ・復旧設計](backup-restore.md) | 🟡 中級 | 30分 | 永続データ、復元試験、復旧目標 |
| [運用ランブック索引](runbooks/README.md) | 🟡 中級 | 30分〜 | 停止・遅延・disk・memory・監視停止時の切り分け |
| [CPU 高負荷演習記録](incidents/cpu-high-drill.md) | 🟡 中級 | 15分 | 模擬障害の再現、確認、復旧、再発防止 |
| [演習一覧](drills/README.md) | 🟡 中級 | 15分 | 構築演習 B-1〜B-4 と復旧演習 D-1 / D-2 |
...
| [LogQL クエリ集](loki-queries.md) | 🟡 中級 | 20分 | dashboardと運用で使うLogQLの例 |
| [検証証跡台帳](evidence/README.md) | 全員 | 20分 | コード実装と実環境での実測を区別する検証状況 |
| [ローカル証跡採録ガイド](evidence/local-evidence-quickstart.md) | 🟡 中級 | 60分〜 | Grafana / Loki / Alertmanager / D-1を実測証跡にする手順 |
| [2〜3 分デモ収録ガイド](demo-capture-guide.md) | 🟡 中級 | 30分 | 配備、故障注入、通知、復旧の短尺収録手順 |
...
| [Windows サーバー構築案件パック](build-package-windows/README.md) | 既存監視基盤へ Windows Server を監視対象ホストとして追加する設計・パラメータ・手順一式（[初心者ガイド](build-package-windows/beginner-guide.md)付き。Ansible 対応 role・central 側ネットワーク拡張・ログ集約経路は未実装） |
...
| [DHCP サーバー構築案件パック](build-package-dhcp/README.md) | 検証用LANセグメント（`192.168.50.0/24`）へIPv4アドレスを払い出す isc-dhcp-server を新規ホスト `dhcp-01` へ構築する設計・パラメータ・手順一式（[初心者ガイド](build-package-dhcp/beginner-guide.md)付き）。新規Ansible role `dhcp_server` と専用playbook `ansible/playbooks/dhcp.yml` を追加し、`ansible-lint --offline`（production profile、0 failure）と `--syntax-check` をローカルでPASS済み。中央Prometheusへのnode_exporter登録はLinuxホストのため未実装ブロッカーなし。2026-09-04にAI支援セッションのサンドボックスコンテナ上（network namespace + vethでDORAを模擬、VM/実機ではない）で`dhcp_server` role単独適用とDORA・固定予約・プール枯渇・リース更新解放・バックアップ復元を実測し、必須31 IDのうち22件PASS（[結果票](evidence/2026-09-04-dhcp-build-validation.md)）。VM/実機での正本実演、`common` role込みのセキュリティ強化、中央監視統合はまだ未実施 |
| [WSUS サーバー構築案件パック](build-package-wsus/README.md) | 既存の AD ドメイン（`corp.example.test`）へ WSUS（Windows Server Update Services）サーバーを 1 台追加し、グループポリシーによる更新プログラムの集中管理を実現する設計・パラメータ・手順一式（[初心者ガイド](build-package-wsus/beginner-guide.md)付き）。Windows 版・AD 版パックで「実務では推奨だが対象外」としていた WSUS/グループポリシー集中管理を埋める案件パック。中央監視統合は他パックと同じ理由で未実装、実ホストでの構築・試験実績も未実装 |
...
| [変更管理ミニ運用](change-management.md) | PR / Issue で目的、影響範囲、ロールバックを記録する運用 |
| [latency-spike ランブック](runbooks/latency-spike.md) | `/healthz` p95 が 500ms を越えた際の切り分け |
| [監視の監視ランブック](runbooks/alertmanager-down.md) | Alertmanager / blackbox-exporter 停止時の対応 |
| [ディスク逼迫ランブック](runbooks/disk-full.md) | ファイルシステム使用率 85% 超過時の切り分け |
| [メモリ逼迫ランブック](runbooks/memory-pressure.md) | メモリ使用率 90% 超過時の切り分け |
| [スナップショット命名規則](backup-naming.md) | バックアップアーティファクトのタグと命名 |
| [インシデント周知テンプレ](incident-comms.md) | Slack へ流す状態遷移ごとの定型文 |

## ダッシュボード機能

**一言でいうと**: 画面に何が出るかの一覧です。CPU、メモリ、ディスク、ネットワーク、実行中のプロセスを1画面で見られます。

![Server Monitor Dashboard（Linux(WSL2) 上での実行画面、2026-08-18）](screenshot.png)

> **この画像について**: 上の画面は **Linux(WSL2) 上で実際に起動して撮影したもの**（[実測証跡](evidence/2026-08-18-local-observability.md)）。
> 実測済みと未実測の境界は [検証証跡台帳](evidence/README.md) を参照。
...
配備手順書（[docs/deployment.md](deployment.md)）は **Ansible 版（[docs/deployment-ansible.md](deployment-ansible.md)）へ移行済み** で、リファレンス扱い。
...
実hostへ適用する前に、[完全なAnsible配備手順](deployment-ansible.md)の前提、
...
引き渡し文書は[Ansible自動化基盤構築案件パック](build-package-ansible/README.md)にまとめている。
...
個々の設定内容は [docs/aws-architecture.md](aws-architecture.md) を参照。

このコードが AWS で稼働した実績や実費を意味するものではない。
ALB 配下の各 EC2 に同居するローカル監視データは、中央の正本としない。
本番相当では外部 probe（外から定期的に叩いて応答を確かめること）と中央保存を追加する。詳細は [docs/aws-architecture.md](aws-architecture.md)、
[docs/cost-report.md](cost-report.md)、[docs/evidence/README.md](evidence/README.md) を参照。
...
D-1 はローカルでRTO 13秒（[2026-08-19](drills/logs/2026-08-19-D-1.md)）、
使い捨て環境のE2E（ephemeral runner）でRTO 1秒（[2026-08-22](evidence/2026-08-22-full-stack-e2e.md)）を実測した。
...
実施計画と振り返りテンプレは [docs/drills/](drills/) を、演習由来の改善履歴は
[docs/backup-restore.md](backup-restore.md) を参照。
...
[変更管理ミニ運用](change-management.md) のチェック項目を使う。証跡採録は
[Evidence capture Issue](../.github/ISSUE_TEMPLATE/evidence-capture.yml) と
[ローカル証跡採録ガイド](evidence/local-evidence-quickstart.md) に沿って記録する。
...
クエリ例は [LogQL クエリ集](loki-queries.md) に整理した。
...
詳細は [構築・配備手順](deployment.md) を参照してください。
...
> **B-1 〜 B-4 はすべて実行済み**です。
> [B-1](drills/logs/2026-08-24-B-1.md) 5 PASS（220M→457M の online 拡張）、
> [B-2](drills/logs/2026-08-24-B-2.md) 9 PASS、
> [B-3](drills/logs/2026-08-24-B-3.md) 7 PASS・RTO 0.149 秒、
> [B-4](drills/logs/2026-08-24-B-4.md) 6 PASS / 3 SKIP-ENV（VLAN 部のみ kernel 都合で未検証）。
> B-1 と B-4 は実行を試みた結果、それぞれ Ansible の版依存と Docker の
> ネットワーク制約で**そのままでは動かない**ことが分かり、直したうえで通しています。
> 経緯は[証跡の索引](evidence/README.md)に書いています。
...
- **恒久的に稼働しているホストがまだありません。** 再起動後の永続性、24 / 72 時間
  ...
  結果票を自動生成する仕組みは
  [10 立ち上げと受け入れ試験](build-package/10-host-bringup-and-acceptance.md)
  に用意しています。
...
- `site.yml`一括構築・冪等性、runner内network/UFW/待受、backup restoreは[2026-08-22の自動E2E](evidence/2026-08-22-full-stack-e2e.md)でPASSです。
