...
今回の応答・認証・計画停止と再開・終了・説明を確認したら、最初の実習を閉じて構いません。追加の改善案は次回候補に残します。確認が未実施なら成功にせず、安全に保留できる対象状態と再開条件を記録します。時間だけを理由に、認証未確認のサービスを残して離れないでください。

確認を繰り返したくなる場合は [手放して進める運用キット](work-completion/README.md) の作業カードと60秒手順を使えます。45分・2回は見直し用の仮値で、必要な試験の省略や障害対応の打切りには使いません。
