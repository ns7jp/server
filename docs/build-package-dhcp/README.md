...
| 案件 ID | 対象 | 現在の引き渡し判定 |
| --- | --- | --- |
| `SM-DHCP-001` | Ubuntu Server 24.04 LTS の検証用 VM 1 台（新規・論理ホスト名 `dhcp-01`）へ isc-dhcp-server を構築し、検証用LANセグメント `192.168.50.0/24` 向けにDHCPv4のリース払い出し（動的プール・固定予約）を提供する | **`NOT READY`** — VM/実機での正本実演は未着手。2026-09-04に**2つの独立したAI支援セッション**がそれぞれサンドボックスコンテナ上で実機検証を実施した。(1) `labs/dhcp-lab/`（network namespace + veth隔離ラボ、`common`ロールも含めて適用）で31 ID中27 ID`PASS`（commit`ebcae209`、[結果票](../evidence/2026-09-04-dhcp-build-validation-netns-lab.md)、[ネットワーク結果票](../evidence/2026-09-04-network-host-validation-dhcp-netns-lab.md)）。2026-09-07には同じ種類のnetnsラボを別commit（`0974872`）で再構築し、DIT-10（監視統合、`up{host="dhcp-01"}=1`を実測）・DNW-03（`dhcp-01`自身の名前解決）の2 IDをPASSへ切り替え（[追補](../evidence/2026-09-07-dhcp-dit10-dnw03-followup.md)）。[11-work-result-report.md](11-work-result-report.md)のとおり別commitの結果は合算しないため、この2 IDと上記27 IDを足した「29/31」という単一の合計値では扱いません。(2) セッション自身のコンテナをDocker bridge越しに`dhcp-01`役として使う構成（`common`ロールは安全上の理由で未適用）で31 ID中22 ID`PASS`（[結果票](../evidence/2026-09-04-dhcp-build-validation.md)）。いずれもVM/実機の正本を置き換えるものではなく、独立した実ホストでの受け入れは引き続き`NOT RUN` |
...
| G3 試験完了 | 対象ホストの必須31 ID（`DUT-01〜05`、`DIT-01〜11`、`DST-01〜06`、`DNW-01〜09`）がすべて `PASS` | 部分実施 — netnsラボ版でcommit`ebcae209`が27/31`PASS`（[結果票](../evidence/2026-09-04-dhcp-build-validation-netns-lab.md)）、別commit`0974872`でDIT-10・DNW-03の2 IDを追加`PASS`（別commitのため合算しない。[追補](../evidence/2026-09-07-dhcp-dit10-dnw03-followup.md)）、コンテナ内bridge版で22/31が`PASS`（[結果票](../evidence/2026-09-04-dhcp-build-validation.md)）。独立した実ホストでの完全な`PASS`は`NOT READY` |
