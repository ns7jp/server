...
> **B-2 / B-3 は実コンテナで実行済み**（[B-2](docs/drills/logs/2026-08-24-B-2.md) 9 PASS、
> [B-3](docs/drills/logs/2026-08-24-B-3.md) 7 PASS、RTO 0.149 秒）。
> **B-1 は未実行**（device-mapper が要る）。
> **B-4 は未実行** — 実行を試みて構成上の欠陥が 2 件見つかり、現在の compose では
> 成立しないことが分かりました。理由は[証跡の索引](docs/evidence/README.md)に書いています。
...
- **B-2 / B-3 は実コンテナで実行した証跡があります。B-1 と B-4 にはありません。**
  B-4 は実行を試みた結果、現在の compose では成立しないことが分かっています
  （[理由](docs/evidence/README.md)）。いずれにせよラボ環境（loop device /
  コンテナ）での演習なので、物理ディスク、物理スイッチ、VLAN 対応スイッチの
  設定は対象外です。
