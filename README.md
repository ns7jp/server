...
| 文書 | 内容 |
| --- | --- |
| [Ansible自動化基盤構築案件パック](docs/build-package-ansible/README.md) | 監視アプリではなく、`common` / `docker` roleと新設した`ansible/playbooks/foundation.yml`（既存roleの組み合わせ）自体を案件の成果物として設計・試験・引き渡しする一式（[初心者ガイド](docs/build-package-ansible/beginner-guide.md)付き）。role設計・変数の優先順位・冪等性・複数OS対応が主題。2026-09-04に手元Hyper-VのVM 2台（Ubuntu・AlmaLinux 9.7）でフェーズ1・フェーズ2の構築・冪等性を実測PASS（[フェーズ1](docs/evidence/2026-09-04-ansible-foundation-build.md) / [フェーズ2](docs/evidence/2026-09-04-ansible-foundation-el9-build.md)）。実行して見つけた欠陥2件は[欠陥台帳](docs/evidence/defects-found.md)#30・#31に記録。フェーズ2のVMは再利用環境のため、最小公開の証跡は専用新規VMでの再実施が必要 |
